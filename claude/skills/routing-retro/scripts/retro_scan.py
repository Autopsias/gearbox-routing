...
Walks session transcript files (default layout: <projects-dir>/*/<session>.jsonl)
and emits per-session JSON: model mix (by abstract TIER, not raw model id), token
breakdown, cost (computed against the SSOT's ACTIVE PROVIDER `prices:` row),
...
stdlib-only (no PyYAML — regex parse of the SSOT, matching the drift guard's house
rule of not requiring a YAML dependency). Read-only: writes nothing anywhere.
...
optionally floor-bounded by --since (file mtime). Sessions whose records carry an
`entrypoint` starting with "sdk" (headless Agent-SDK runs) and any --exclude-session
id (pass the CURRENT session's id!) are skipped and reported in `skipped`.

Pricing is resolved against `providers.<provider>.models` (raw model id -> tier) and
`prices.<provider>.<tier>` (tier -> $/MTok in/out) from the SSOT. `--provider`
defaults to the SSOT's `active_provider:`; override only to price a scan against a
different provider's rate card (rare — e.g. "what would this have cost on provider X").
"""
...
# Cache pricing multipliers relative to the SSOT input rate (Anthropic-style cache
# semantics: cache read = 0.1x input; cache write = 1.25x (5m TTL) / 2x (1h TTL)
# input). This ratio is an industry-common convention, not a per-provider SSOT
# value — if a provider's cache economics differ materially, extend this script
# rather than bending the SSOT schema to carry it.
CACHE_READ_X = 0.10
CACHE_W5M_X = 1.25
CACHE_W1H_X = 2.00
...
TIERS = ("cheap_fast", "workhorse", "frontier_reasoner")


def _sub_block(text, key, indent):
    """Return the raw text of a `<indent spaces><key>:` block's BODY (everything
    after that header line up to the next line at <= indent), tolerating a
    trailing inline comment on the header line itself. None if not found."""
...
def parse_ssot(ssot_path, provider_override=None):
    ...
    m = re.search(r"^active_provider:\s*(\S+)", text, re.M)
    if not m:
        sys.exit("FATAL: no `active_provider:` scalar in " + ssot_path)
    active_provider = provider_override or m.group(1)

    # providers.<active_provider>.models: raw model id -> tier
    providers_block = _sub_block(text, "providers", 0)
    if providers_block is None:
        sys.exit("FATAL: no `providers:` block in " + ssot_path)
    prov_body = _sub_block(providers_block, active_provider, 2)
    if prov_body is None:
        sys.exit(f"FATAL: no `providers.{active_provider}:` block in " + ssot_path)

    models_block = _sub_block(prov_body, "models", 4)
    model_to_tier = {}
    if models_block:
        for mo in re.finditer(r"^\s{6}(\w+):\s*(\S+)", models_block, re.M):
            tier, model_id = mo.group(1), mo.group(2)
            if tier in TIERS:
                model_to_tier[model_id.rstrip(",")] = tier

    # prices.<active_provider>.<tier>: {in, out}
    prices_block = _sub_block(text, "prices", 0)
    if prices_block is None:
        sys.exit("FATAL: no `prices:` block in " + ssot_path)
    price_body = _sub_block(prices_block, active_provider, 2)
    if price_body is None:
        sys.exit(f"FATAL: no `prices.{active_provider}:` block in " + ssot_path)

    prices = {}
    for mo in re.finditer(r"^\s{4}(\w+):\s*\{\s*in:\s*([\d.]+),\s*out:\s*([\d.]+)", price_body, re.M):
        tier = mo.group(1)
        if tier in TIERS:
            prices[tier] = (float(mo.group(2)), float(mo.group(3)))

    if not prices:
        sys.exit(f"FATAL: parsed zero tier rows from prices.{active_provider} — parser or SSOT broken")

    return active_provider, prices, model_to_tier


def resolve_tier(raw_model_id, model_to_tier):
    """Map a raw model id seen in a transcript to an abstract tier, via the active
    provider's `models:` map (exact match, then substring, both directions —
    ids sometimes carry version/date suffixes the SSOT entry omits or vice versa)."""
    if not raw_model_id:
        return None
    if raw_model_id in model_to_tier:
        return model_to_tier[raw_model_id]
    low = raw_model_id.lower()
    for mid, tier in model_to_tier.items():
        mid_low = mid.lower()
        if mid_low and (mid_low in low or low in mid_low):
            return tier
    return None
...
def scan_session(path, prices, model_to_tier):
    s = {
        ...
        "tiers": {},           # tier -> {messages, in, out, cache_w, cache_r, cost_usd}
        "unknown_models": {},  # raw id -> message count (no tier resolved)
        "cost_usd": 0.0,
        "tool_errors": 0,
        "api_errors": 0,
    ...
    with open(path, encoding="utf-8") as f:
        for line in f:
            ...
            if rtype == "user":
                ...
                tier = resolve_tier(msg.get("model"), model_to_tier)
                usage = msg.get("usage") or {}
                if tier is None or tier not in prices:
                    key = msg.get("model") or "?"
                    s["unknown_models"][key] = s["unknown_models"].get(key, 0) + 1
                else:
                    row = s["tiers"].setdefault(tier, {"messages": 0, "in": 0, "out": 0, "cache_w": 0, "cache_r": 0, "cost_usd": 0.0})
                    row["messages"] += 1
    ...
    if ts_first and ts_last:
        try:
            ...
            pass
    total_msgs = sum(r["messages"] for r in s["tiers"].values())
    for r in s["tiers"].values():
...
def main():
    ...
    ssot_path = os.path.normpath(args.ssot)
    active_provider, prices, model_to_tier = parse_ssot(ssot_path, args.provider)
    ...
    for fp in files:
        ...
        if sid in args.exclude_session:
            ...
            continue
        s = scan_session(fp, prices, model_to_tier)
    ...
    out = {
        "note": (
            f"Costs are computed against the SSOT prices.{active_provider} block "
            f"({ssot_path}) with standard cache multipliers "
            f"(read {CACHE_READ_X}x-in, write {CACHE_W5M_X}x/{CACHE_W1H_X}x-in). "
            "Effort is NOT recorded in transcripts — it must be INFERRED "
            "(local settings + env overrides + receipts), never claimed as measured."
        ),
