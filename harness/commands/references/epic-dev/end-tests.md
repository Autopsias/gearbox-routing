...
- [STEP 3: Verify Epic Readiness](#step-3-verify-epic-readiness)
- [STEP 4: Session Management](#step-4-session-management)
- [STEP 5: Execute Phase Loop](#step-5-execute-phase-loop)
- [WAIVER HANDLING](#waiver-handling)
- [Waiver](#waiver)
- [STEP 6: Completion Summary](#step-6-completion-summary)
...
- If no epic_number provided, error with: "Usage: /epic-dev <epic-number> --end [--yolo] [--resume]"
...
Load output folder from config (default: `_bmad-output`)
...
IF decision == "Stop":
  HALT with: "Complete remaining stories, then run: /epic-dev {epic_num} --end"
```
...
3. Execute: Skill(skill='bmad-tea-testarch-nfr')
...
IF nfr_status == "FAIL":
  ...
  IF fail_decision == "Stop and remediate":
    ...
    Output: "Resume with: /epic-dev {epic_num} --end --resume"
    HALT
...
IF NOT --yolo:
  ...
  IF continue_decision == "Stop":
    Output: "Stopping at Phase 1. Resume with: /epic-dev {epic_num} --end --resume"
    HALT
...
3. Execute: Skill(skill='bmad-tea-testarch-test-review')
...
IF NOT --yolo:
  ...
  IF continue_decision == "Stop":
    Output: "Stopping at Phase 2. Resume with: /epic-dev {epic_num} --end --resume"
    HALT
...
3. Execute: Skill(skill='bmad-tea-testarch-trace')
...
ELSE IF decision == "CONCERNS":
  ...
  ELSE IF concerns_decision == "Address gaps":
    ...
    Output: "Re-run after fixes: /epic-dev {epic_num} --end"
    HALT
...
────────────────────────────────────────────────────────────────────────────────
  NEXT MANDATORY STEP: Interactive UAT
────────────────────────────────────────────────────────────────────────────────

  Run: /epic-dev {epic_num} --uat
  Epic CANNOT be marked 'done' until UAT passes.
...
IF gate_decision == "PASS":
  - Run UAT: /epic-dev {epic_num} --uat
  - After UAT passes: /epic-dev {epic_num} (to mark epic done)
...
ELSE IF gate_decision == "FAIL" OR gate_decision == "FAIL (FORCED)":
  - Address blocking issues before UAT
  - Re-run: /epic-dev {epic_num} --end
...
**Purpose:** Track 3-phase post-epic validation workflow with task dependencies and status visibility.

### INITIALIZATION: Create Phase Tasks at Workflow Start

When starting epic end tests (not resuming), create all 3 phase tasks:

```
# At STEP 5 start (before phase execution loop):

IF NOT resuming_session:
  # Create all 3 phase tasks with blockedBy dependencies
  TaskCreate(
    subject="Phase 1: NFR Assessment - Epic {epic_num}",
    description="Assess non-functional requirements (performance, security, reliability, maintainability) using opus model",
    activeForm="Assessing NFRs"
  ) -> task_id_1

  TaskCreate(
    subject="Phase 2: Test Quality Review - Epic {epic_num}",
    description="Review test quality against best practices using sonnet model",
    activeForm="Reviewing test quality"
  ) -> task_id_2
  TaskUpdate(taskId=task_id_2, addBlockedBy=[task_id_1])

  TaskCreate(
    subject="Phase 3: Quality Gate Decision - Epic {epic_num}",
    description="Make quality gate decision (PASS/CONCERNS/FAIL/WAIVED) using opus model",
    activeForm="Making quality gate decision"
  ) -> task_id_3
  TaskUpdate(taskId=task_id_3, addBlockedBy=[task_id_2])

  Output: "Created 3 phase tasks for Epic {epic_num} end tests"
END IF
```
...
```
# When entering Phase 1 (NFR Assessment):
...
# When entering Phase 2 (Test Quality Review):
...
# When entering Phase 3 (Quality Gate):
...
```
TaskUpdate(taskId=task_id_3, metadata={
  "gate_decision": "WAIVED",
...
At workflow completion, display task summary:
...
Output: "Task Summary:"
FOR task IN tasks:
  Output: "  [{task.status}] {task.subject}"
END FOR
```
