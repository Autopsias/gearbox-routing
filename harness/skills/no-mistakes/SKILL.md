...
## Optional: `--rules` (rule-adherence check)

This is a skill-level flag, not an `axi run` flag - it never runs by default,
only when the user's `/no-mistakes` request explicitly includes `--rules` (or
an equivalent ask, e.g. "check this against my hard rules"). It is entirely
separate from the `axi run` pipeline above: it does not gate, block, fix, or
otherwise affect that pipeline's outcome. It is an informational pass you run
and report alongside it.

When `--rules` is requested:

1. Read `~/.claude/CLAUDE.md` fresh, at invocation time, and extract every
   top-level bullet under its `## Behavior` heading. Never hardcode these
   rules anywhere - they change over time, and this step must always reflect
   the current file, not a snapshot. Assign each bullet an id `r1`, `r2`, ...
   `rN` in the order it appears.
2. Build `args = { rules: [{ id, text }, ...], target: <description of the
   pending diff under review, e.g. the commit range or file list> }`.
3. Invoke `Workflow { scriptPath: '~/.claude/workflows/rule-adherence.js' }`
   with those args.
4. The workflow fans out one sonnet verifier agent per rule, then one sonnet
   skeptic agent per violated finding (biased to refute; defaults to refuted
   when uncertain). It returns `{ confirmed: [...], checked: N }` - only
   findings that survived the skeptic.
5. Surface the `confirmed` findings alongside your existing report, each with
   its rule id, evidence, and confidence. If `confirmed` is empty, say so
   explicitly ("no rule-adherence violations found across N rules checked")
   rather than staying silent about having run the check.

**Token cost, stated honestly:** this is one extra sonnet agent call per
Behavior rule, plus one more per violated finding (the skeptic pass - not
per rule, only per hit). With a handful of Behavior rules this is cheap;
if that list grows long, say so before running it.
