...
## Checklist (all 4 phases)

- [ ] **Interview** — same shared interview opening as `/grill-me`, waiting for feedback on each question
- [ ] **Domain-awareness side-effects** — challenge against the glossary, sharpen fuzzy language, cross-reference with code
...
## After the session — adversarial review (default)

Same shared protocol as `/grill-me` — `Read ~/.claude/skills/_shared/grill-adversarial-review.md` ("After the session — adversarial review" section) for the exact steps. One note specific to this skill: the `CONTEXT.md` / ADR updates stay where they are — they are not the review target; the plan/design is.
